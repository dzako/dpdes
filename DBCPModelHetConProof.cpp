...
void integrate(int approach){
  ...
  PolyBdInputReader<RealPolynomialBound> inputReader("polybd.in");
...
//    std::cout << "step #" << i << " ";
    ...
    if(i % 100 == 0){
      ...
      modes(currentSet, set.getPerturbationParams() );
      currentSet.close();
    }
  }
...
void proveStableFixedPoint(){
  ...
  finder.inflateTrappingRegion(Qbox, eigenvaluesRe, T, dfPoint, FevaluatedAtFP, std::cout );
}
...
int main(int argc, char * argv[]){
  ...
  if(argc != 3){
      std::cerr << "Number of arguments wrong.\nUsage: ./DBCPModelHetConProof [integrate|fixedpoint] approach_number\n" <<
