...
public:
  ...
  Burgers(RealType nu_) : nu(nu_), S_DISSIPATIVE(-1), piOver_l(1.){
    m_p=2.;
    m_d=1.;
    m_r=1.;
  ...
  Burgers(int m, int M, RealType nu_) : SubspaceType(m, M), nu(nu_), S_DISSIPATIVE(-1), piOver_l(1.){
    m_p=2.;
    m_d=1.;
    m_r=1.;
    m_sufficientlyLarge=m_d+m_p+1;
...
public:
    ...
    GL(RealType nu_) : nu(nu_), S_DISSIPATIVE(-0.01), piOver_l(1.){
      m_p=2.;
      m_d=1.;
      m_r=0.;
      m_sufficientlyLarge=m_d+m_p+1;
    ...
    GL(int m, int M, RealType nu_) : SubspaceType(m, M), nu(nu_), S_DISSIPATIVE(-0.01), piOver_l(1.){
      m_p=2.;
      m_d=1.;
      m_r=0.;
      m_sufficientlyLarge=m_d+m_p+1;
...
public:
    ...
    KS(RealType nu_) : nu(nu_), S_DISSIPATIVE(-0.01), piOver_l(1.){
      m_p=4.;
      m_d=1.;
      m_r=1.;
      m_sufficientlyLarge=m_d+m_p+1;
    ...
    KS(int m, int M, RealType nu_) : SubspaceType(m, M), nu(nu_), piOver_l(1.){
      m_p=4.;
      m_d=1.;
      m_r=1.;
      m_sufficientlyLarge=m_d+m_p+1;
...
public:
    ...
    SH(RealType nu_) : nu(nu_), S_DISSIPATIVE(-0.01), piOver_l(1.){
      m_p=4.;
      m_d=1.;
      m_r=0.;
      m_sufficientlyLarge=m_d+m_p+1;
    ...
    SH(int m, int M, RealType nu_) : SubspaceType(m, M), nu(nu_), S_DISSIPATIVE(-0.01), piOver_l(1.){
      m_p=4.;
      m_d=1.;
      m_r=0.;
      m_sufficientlyLarge=m_d+m_p+1;
...
template<class PolyBdT>
class CH : public PolyBdT::SubspaceType, public capd::jaco::DPDEDefinition<PolyBdT>{
public:
    typedef PolyBdT PolyBdType;
    typedef typename PolyBdType::SubspaceType SubspaceType;
    typedef typename PolyBdType::ComplexScalarType ComplexType;
    ...
    CH(RealType nu_) : nu(nu_), S_DISSIPATIVE(-0.01){
      m_p=4.;
      m_d=1.;
      m_r=2.;
      m_sufficientlyLarge=m_d+m_p+1;
    ...
    CH(int m, int M, RealType nu_) : SubspaceType(m, M), nu(nu_), S_DISSIPATIVE(-0.01){
      m_p=4.;
      m_d=1.;
      m_r=2.;
      m_sufficientlyLarge=m_d+m_p+1;
    ...
    RealType ni(int k) const{  IndexType j=array2modeIndex(k); RealType sqN=j.squareEuclNorm() * piOver_l * piOver_l * piOver_l * piOver_l; return (j.isZero() ? 0 : 1 - nu / sqN); }

    RealType ni(const IndexType& k) const{ RealType sqN=k.squareEuclNorm() * piOver_l * piOver_l * piOver_l * piOver_l; return (k.isZero() ? 0 : 1 - nu / sqN); }
    ...
    ///i is position in an array of a mode.
    RealType lambda(int i) const{ IndexType j=array2modeIndex(i); RealType sqN=j.squareEuclNorm() * piOver_l * piOver_l; return -sqN * (sqN - nu); }

    ///k is the index of mode, lambda_k from the paper.
    RealType lambda_k(int k) const{ return - k * k * piOver_l * piOver_l * (k * k * piOver_l * piOver_l - nu); }

    RealType lambda_k(const IndexType& k) const{ RealType sqN = k.squareEuclNorm() * piOver_l * piOver_l; return -sqN * (sqN - nu);}
...
template<class PolyBdT>
class DBCP : public PolyBdT::SubspaceType, public capd::jaco::DPDEDefinition<PolyBdT>{
public:
    typedef PolyBdT PolyBdType;
    typedef typename PolyBdType::SubspaceType SubspaceType;
    typedef typename PolyBdType::ComplexScalarType ComplexType;
    ...
    DBCP(RealType nu_) : nu(nu_), S_DISSIPATIVE(-0.01){
      m_p=4.;
      m_d=1.;
      m_r=2.;
      m_factorMaxM = 10;
    ...
    DBCP(int m, int M, RealType nu_) : SubspaceType(m, M), nu(nu_), S_DISSIPATIVE(-0.01){
      m_p=4.;
      m_d=1.;
      m_r=2.;
      m_factorMaxM = 10;
    ...
    RealType ni(int k) const{  IndexType j=array2modeIndex(k); RealType sqN=j.squareEuclNorm() ; return (j.isZero() ? 0 : piOver_l * piOver_l * piOver_l * piOver_l - piOver_l * piOver_l * nu / sqN - (nu * sigma) / (sqN * sqN)); }

    RealType ni(const IndexType& k) const{ RealType sqN=k.squareEuclNorm()  ; return (k.isZero() ? 0 : piOver_l * piOver_l * piOver_l * piOver_l - piOver_l * piOver_l * nu / sqN - (nu * sigma) / (sqN * sqN)); }
    ...
    ///i is position in an array of a mode.
    RealType lambda(int i) const{ IndexType j=array2modeIndex(i); RealType sqN=j.squareEuclNorm() * piOver_l * piOver_l; return -sqN * (sqN - nu) - nu * sigma; }

    ///k is the index of mode, lambda_k from the paper.
    RealType lambda_k(int k) const{ return - k * k * piOver_l * piOver_l * (k * k * piOver_l * piOver_l - nu) - nu * sigma; }

    RealType lambda_k(const IndexType& k) const{ RealType sqN = k.squareEuclNorm() * piOver_l * piOver_l; return -sqN * (sqN - nu) - nu * sigma;}
    ...
    //TODO: different for this equation
    int maximumPoint(const RealType& h, int r, int k) const{
      if( -4*rightBound(h)*k*k*k*k*piOver_l*piOver_l*piOver_l*piOver_l + 2*nu*rightBound(h)*k*k*piOver_l*piOver_l + r <= 0 && k*k >= nu / (4 * piOver_l * piOver_l) ){
          return k;
      }else{
        int c=k;
        while(!(-4*rightBound(h)*c*c*c*c*piOver_l*piOver_l*piOver_l*piOver_l + 2*nu*rightBound(h)*c*c*piOver_l*piOver_l + r <= 0 && c*c >= nu / (4 * piOver_l * piOver_l))){
          c++;
        }
        return c;
      }
...
public:
  ...
  inline ComplexScalarType bound(const ModesContainerType& pb, const IndexType& k, ModesContainerType& out) const{
    ...
    if(k.d() == 1){
      ...
      RealType t = 2. * C_ * C_ * (1. / (2. * s_ - 1.)) * power(1. / ScalarType((M+k[0]) * (M)), s_ - 0.5) * unit;
      ...
      if(k.d() == 2){
        ...
        RealType t = C_ * C_   *  (power(2, s_) / divisor)  *  IndexType::template harmonicSumK_1<RealType, IndexRangeType, VNormType>(range, s_)  * unit;
    ...
    if(k.d() == 1){
      ...
      RealType t = 2. * C1 * C2 * sqrt(1. / ( (2. * s1 - 1.) * (2. * s2 - 1.) )) *
          (power(1. / ScalarType(M + k[0]), s1 - 0.5) * power(1. / ScalarType(M), s2 - 0.5) +
           power(1. / ScalarType(M + k[0]), s2 - 0.5) * power(1. / ScalarType(M), s1 - 0.5)) * unit;
   ...
   */
  inline void addBound(const ModesContainerType& pb, ModesContainerType& out, int range = full, bool doubleTheRange = false) const{
    IndexType i;
    ...
    if(doubleTheRange && (range == finitePart || range == finitePartPlusTail) ) r.doubleK_1Range();
   ...
   * then the ifs like if(index.d() == 1) are not needed.
   */
  inline virtual RealType estimatePolynomialBoundForTheInfinitePart(const PolyBdType& pb) const{
...
//        for(k_1=this->firstWithinRange(irK_1), k_1.l=k.l; !k_1.limitReached(irK_1); k_1.inc(irK_1, true)){
  ...
  inline RealType estimateCG(const RealType& h, const PolyBdType& x_0, const ModesContainerType& W_2){
  ...
  inline RealType investigate(const RealType& step, const PolyBdType& x_0, const PolyBdType& W_2){
...
*/
  ...
  int changeM(int newM, PolyBdType& T0, PolyBdType& T, bool& mWasChanged, bool guard = false) {
  ...
  bool updateM(PolyBdType& T, PolyBdType& T0, int& L, RealType& dL, int previousL,
      RealType previousdL, bool& validated, bool& farTailValidate, bool& changedLastStep, bool& mWasChanged) {
    int newM = -1;
    ...
    if(!changedLastStep && newM > 0 && previousdL > 0 ) {
      ...
      tailDebug << "validate of the far tail failed, increasing M, newM=" << newM << ", close tail size=" << T.dimension() << "\n";
  ...
  bool updateT(const RealType& h, const PolyBdType& x_0, PolyBdType& W_2, const PolyBdType& gt_b,
      const RealType& newC, int& L, RealType& dL, bool& validated, bool& farTailValidate) {
    bool r = false;
    if(!(newC <= C(W_2))) {
      r = true;
      setCLarger(W_2, __D_2__ * newC);
      RealType Cg = estimateCG(h, x_0, W_2),
               pow;
          ...
          for(int j = -__INFLATE_RADIUS__; j <= __INFLATE_RADIUS__; j++) {
            for(int k = 0; k < index.d(); k++){
              ...
              if(W_2.irFiniteTail.withinRange(tindex) && j != 0) {
                ...
                inflates[this->mode2array(tindex, true)] *= 1. + (__INFLATE_C__ - 1.) / ScalarType(abs(j)).rightBound();
              }
            }
          }
        }
...
//    if(L>=M) tailDebug<<"T(L,1)="<<W_2(L, 1)<<"\n";
  ...
  #if !__CONSTANT_M__
          updateM(W_2, x_0, L, dL, previousL, previousdL, validated, farTailValidate, changedLastStep, mWasChanged);
  #else
          tailDebug<<"previousdL="<<previousdL<<" dL="<<dL<<"\n";
  #endif
        }
    ...
    if(s(m_bt) > s(x_0)) {
      ...
      if(!(C(x_0) >= Cb * power(M, s(x_0) - s(m_bt)))) {
        ...
        if(t) {
          recalculateN = false;
  #if !__CONSTANT_M__
          tailDebug<<"L="<<L<<"\ndL="<<dL<<"\nC(T)/Cg="<<power(C(W_2) / Cg, 1. / (s(W_2) - s(m_gt)))<<"\n";
          updateM(W_2, x_0, L, dL, previousL, previousdL, validated, farTailValidate, changedLastStep, mWasChanged);
  #else
          tailDebug<<"previousdL="<<previousdL<<" dL="<<dL<<"\n";
  #endif
...
public:
  ...
  inline void rightHandSide(int i, const GridsContainerType& grids, const ModesContainerContainerType& modes,
    ...
    */
   inline void CalculateConvolutionDirectly(const ModesContainerType& in, ModesContainerType& out, int range = full, bool doubleTheRange = false){
     ((DPDEContainer&)out).multiply(in, in);
     if( !in.infiniteDimensional ){
       IndexType k, k_1;
       ScalarT sum;
 ...
 //        for(k_1=this->firstWithinRange(irK_1), k_1.l=k.l; !k_1.limitReached(irK_1); k_1.inc(irK_1, true)){
       ...
       if( doubleTheRange && (range == finitePart || range == finitePartPlusTail) ) irK.doubleK_1Range();
 ...
 //        for(k_1=this->firstWithinRange(irK_1), k_1.l=k.l; !k_1.limitReached(irK_1); k_1.inc(irK_1, true)){
       ...
       if(range != redundantRange) addBound(in, out, range, doubleTheRange);
       estimatePolynomialBoundForTheInfinitePart(in, out);
     }
   }
...
//       fftDebug.log = false;
    ...
    * then the ifs like if(index.d() == 1) are not needed.
    */
   inline virtual RealType estimatePolynomialBoundForTheInfinitePart(const PolyBdType& u) const{
       ...
       if(index.d() == 1){
         ...
         boundSquare = 2. * C * (C * twoToS / ((s - 1.) * power(M, s - 1)) + 0.5 * twoToS * twoToS * C / (power(2 * M + 1, s)) +
                 twoToS * A);
         ...
         if(s1 == s2){ //simpler case
           ...
           std::cerr << "error in estimatePolynomialBoundForTheInfinitePart(u) function. Numbers s1 and s2 should be equal\n";
           throw std::runtime_error("error in estimatePolynomialBoundForTheInfinitePart(u) function. Numbers s1 and s2 should be equal\n");
         }
         ///!add the bound for the infinite part
         bound += C1 * C2 / ( (s1 - 1.) * power(M, s1 - 1) ) + C1 * C2 / ( (s2 - 1.) * power(M, s2 - 1) );
  ...
  inline virtual void N(const ModesContainerType& in, ModesContainerType& out, int range = full){
    if(useFFT){
      ...
      if(range != redundantRange) this->multiplyComponents(out);
      if(in.infiniteDimensional)
        this->multiplyComponentsFarTail(out);
      ComplexScalarType constant = Ncoeff();
